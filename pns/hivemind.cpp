...
HiveMind::HiveMind(Config conf_in) :
    ...
    // Invoked by our beldexd to notify of a new block:
    ...
    omq_.add_category("admin", oxenmq::AuthLevel::admin)
            ...
            // - '@' -- the account ID (Bchat ID or closed group ID) to which the message was sent
            //   (33 bytes).
...

    {
        ...
        sd_notify(0, "STATUS=Connecting to beldexd");
        log::info(cat, "Connecting to beldexd @ {}", config.beldexd_rpc.full_address());

        std::promise<void> prom;
        beldexd_ = omq_.connect_remote(
                config.beldexd_rpc,
                [&prom](auto) { prom.set_value(); },
                [&prom](auto, std::string_view err) {
                    try {
                        throw std::runtime_error{"beldexd connection failed: " + std::string{err}};
                    } catch (...) {
        ...
        log::info(cat, "Waiting for beldexd connection...");
        prom.get_future().get();

        prom = {};
        omq_.request(beldexd_, "ping.ping", [&prom](bool success, std::vector<std::string> data) {
            if (success)
                prom.set_value();
            else
                try {
                    std::string err = "beldexd failed to respond to ping:";
                    if (data.empty())
        ...
        prom.get_future().get();
        log::info(cat, "Connected to beldexd");
...
void HiveMind::subs_slow() {
    ...
    omq_.request(beldexd_, "sub.block", nullptr);
