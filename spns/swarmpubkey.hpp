...
template <>
struct hash<pns::SwarmPubkey> {
    size_t operator()(const pns::SwarmPubkey& x) const {
        // A random chunk of the inside of the pubkey is already a good hash without
        // needing to otherwise hash the byte string
        static_assert(
                alignof(pns::SwarmPubkey) >= alignof(size_t) &&
                offsetof(pns::SwarmPubkey, id) % sizeof(size_t) == 0);
