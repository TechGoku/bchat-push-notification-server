...
from pns.core import Config, logger as core_logger
import oxenmq

logger = logging.getLogger("pns")
...
def load_config():
    if "PNS_CONFIG" in os.environ:
        conf_ini = os.environ["PNS_CONFIG"]
        if conf_ini and not os.path.exists(conf_ini):
            raise RuntimeError(f"PNS_CONFIG={conf_ini} specified, but path does not exist!")
    else:
        conf_ini = "pns.ini"
        if not os.path.exists(conf_ini):
            raise RuntimeError(
                "pns.ini does not exist; either create it or use PNS_CONFIG=... to specify an"
    ...
    setting_map = {
        ...
        "hivemind": {
            ...
            "beldexd_rpc": ("beldexd_rpc", lambda x: re.search("^(?:tcp|ipc|curve)://.", x)),
        },
    }
